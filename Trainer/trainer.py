...
class EcgImageToDigitizedTrainer(Trainer):
    def train_batch(self, batch) -> BatchResult:
        ...
        dim_ratio = y[0].nelement() / y[1].nelement() #adjust importance
        y_short = y[0]
        y_long = y[1].reshape(y[1].size(0), 1, y[1].size(1))
        self.optimizer.zero_grad()

        out = self.model(x)
        short_out = out[0]
        long_out = out[1]
        padding_long = long_out.size(2) - y_long.size(2)
        padding_short = short_out.size(2) - y_short.size(2)
        y[0] = F.pad(y_short, (0, padding_short))
        y[1] = F.pad(y_long, (0, padding_long))
        loss = dim_ratio * self.loss_fn(out[0], y[0]) + self.loss_fn(out[1], y[1])
        loss.backward()
        self.optimizer.step()

        num_correct = batch_size * (torch.sum(torch.abs(out[0] - y[0]) < 0.01) + torch.sum(torch.abs(out[1] - y[1]) < 0.01)) / (torch.numel(y[0]) + torch.numel(y[1]))

        return BatchResult(loss.item(), num_correct.item(), 0, 0, 0, 0, 0, 0)
